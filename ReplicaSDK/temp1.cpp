...
DepthMesh::DepthMesh(const std::string& meshFile, const std::string& atlasFolder, bool rS) {
  ...
  if(renderSpherical){
    ...
    shader.AddShaderFromFile(pangolin::GlSlGeometryShader, shadir + "/depth-mesh.geom", {}, {shadir});
    shader.AddShaderFromFile(pangolin::GlSlFragmentShader, shadir + "/depth-mesh.frag", {}, {shadir});
    shader.Link();
...
void DepthMesh::Render(const pangolin::OpenGlRenderState& cam, const Eigen::Vector4f& clipPlane, int lrC) {
  for (size_t i = 0; i < meshes.size(); i++) {
    RenderSubMesh(i, cam, clipPlane, lrC);
  }
}

std::vector<MeshData> DepthMesh::SplitMesh(const MeshData& mesh, const float splitSize) {
...
void DepthMesh::CalculateAdjacency(const MeshData& mesh, std::vector<uint32_t>& adjFaces) {
  struct EdgeData {
...
void DepthMesh::LoadMeshData(const std::string& meshFile) {
  // Load the meshes
...
void DepthMesh::LoadAtlasData(const std::string& atlasFolder) {
  isHdr = false;
